...
func peekFieldType(e *schema.Expr) GoType {
	...
	if e.Field != "" && e.Value != nil && isComparisonOp(e.Op) {
		return GoTypeFromJSONValue(e.Value)
	}
...
func detectOneOfBranches(s *schema.Schema) []OneOfBranch {
	...
	// Group fields by their common prefix pattern (e.g., "payment" -> "Payment")
	...
	for _, f := range s.Fields {
		...
		prefix := parts[0]
		if _, ok := prefixGroups[prefix]; !ok {
			prefixGroups[prefix] = nil
			prefixOrder = append(prefixOrder, prefix)
		}
		prefixGroups[prefix] = append(prefixGroups[prefix], f.Name)
	}

	// Find prefixes shared by 2+ fields — these are likely oneOf groups
	for _, prefix := range prefixOrder {
		fields := prefixGroups[prefix]
		if len(fields) < 2 {
			continue
		}

		groupName := GoFieldName(prefix)
		branchBase := groupName + "Branch"

		// Skip if already tracked from an explicit oneOf rule
		if _, tracked := groups[branchBase]; tracked {
			continue
		}

		groups[branchBase] = &groupCollector{groupBase: groupName}
		order = append(order, branchBase)

		for i, fname := range fields {
			groups[branchBase].branches = append(groups[branchBase].branches, OneOfBranch{
				GroupName: branchBase,
				Branch:    GoFieldName(fname),
				Index:     i,
			})
		}
	}
